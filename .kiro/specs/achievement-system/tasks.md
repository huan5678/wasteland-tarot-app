...
- [ ] 6.2 在使用者登入流程整合成就檢查
  - 在登入 API endpoint 加入成就檢查邏輯
  - 觸發連續簽到相關成就的條件檢查
  - 處理 Bingo 簽到與成就系統的協同
  - _Requirements: 3.2, 3.7_

- [ ] 6.3 在社交互動流程整合成就檢查
  - 在好友新增 API endpoint 加入成就檢查邏輯
  - 在閱讀分享 API endpoint 加入成就檢查邏輯
  - 觸發社交類別成就的條件檢查
  - _Requirements: 3.3, 3.7_

- [ ] 6.4 在 Bingo 活動流程整合成就檢查
  - 在 Bingo 三連線完成時觸發成就檢查
  - 檢查 Bingo 相關成就的解鎖條件
...
- [ ] 7.1 建立成就系統 Zustand Store
  - 建立全域狀態管理 store，管理成就定義與使用者進度
  - 實作查詢成就定義的 action，呼叫後端 API
  - 實作查詢使用者進度的 action，呼叫後端 API
  - 實作獎勵領取的 action，處理 API 呼叫與狀態更新
  - 實作新解鎖成就的暫存與清除邏輯
  - 建立 selector 函數，支援分類篩選與進度計算
  - 整合 persist middleware，快取成就定義至 localStorage
...
- [ ] 8.1 建立成就卡片網格元件
  - 實作成就卡片的視覺呈現，包含圖示、名稱、描述、進度條
  - 實作稀有度視覺效果（邊框顏色、動畫效果）
  - 實作分類篩選功能，支援使用者選擇特定類別
  - 實作搜尋功能，支援關鍵字篩選
  - 實作排序功能（依稀有度、進度、解鎖時間）
  - 整合 PixelIcon 元件顯示成就圖示
  - 使用 Cubic 11 字體顯示文字內容
...
- [ ] 8.2 建立成就詳細資訊 Modal
  ...
  - 顯示完整的解鎖條件與獎勵細節
  - 顯示解鎖時間（如已解鎖）
  - 實作鍵盤導航支援（Tab 鍵、Esc 關閉）
  - _Requirements: 5.6_

- [ ] 8.3 建立成就解鎖通知彈窗元件
  - 實作 Fallout Pip-Boy 風格的解鎖彈窗
  - 顯示成就圖示（帶發光動畫）、名稱、描述、獎勵內容
  - 實作「領取獎勵」與「稍後查看」按鈕
  - 處理多個成就依序顯示的邏輯（每個間隔 2 秒）
  - 整合動畫效果（淡入淡出、縮放）
  - 實作鍵盤焦點陷阱與 Esc 關閉功能
  - 加入 ARIA 標籤以支援螢幕閱讀器
...
- [ ] 8.4 建立進度條與狀態指示元件
  - 實作進度條視覺呈現，顯示當前進度 / 目標進度
  - 實作解鎖狀態徽章（進行中 / 已解鎖 / 已領取）
...
- [ ] 9.2 整合解鎖通知至全域布局
  ...
  - 監聽 store 中的新解鎖成就狀態
  - 處理通知彈窗的顯示與隱藏邏輯
  - 確保通知不與其他 UI 元件（如 Bingo 獎勵）衝突
  - _Requirements: 5.3, 8.4_

- [ ] 9.3 整合成就入口至使用者 Profile 與 Dashboard
  - 在 Profile 頁面加入成就概覽區塊
  - 在 Dashboard 加入「查看成就」快捷連結
  - 顯示已解鎖成就數量與總成就數量
  - 顯示最近解鎖的成就（最多 3 個）
  - _Requirements: 5.1_
