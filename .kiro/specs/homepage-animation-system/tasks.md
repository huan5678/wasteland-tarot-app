...
- [ ] 9.1 實作數字滾動核心邏輯
  - 在 Stats Section component 中整合 `useScrollAnimation` hook
...
- [ ] 9.3 實作背景脈衝效果
  ...
  - 使用 `motionVariants.ts` 定義脈衝 variant（或 inline 定義）
  - 動畫與數字滾動同步開始與結束
  - _Requirements: 5.7_
...
- [ ] 10.1 實作卡片浮入動畫
  - 在 Testimonials Section 中整合 `useScrollAnimation` hook（或 `useStagger`）
...
- [ ] 10.2 實作卡片 hover 效果
  ...
  - Hover 時卡片縮放至 1.02 並增強陰影（Fallout 主題發光效果）
  - Transition duration: 0.3s
  - 使用 `motionVariants.ts` 的預定義 variant（或 inline 定義）
  - _Requirements: 6.3_

- [ ] 10.3 確保圖片載入後再觸發動畫
  - 在 TestimonialCard 內的 avatar 圖片使用 `onLoad` event 偵測載入完成
  - 使用 state 追蹤載入狀態，僅在圖片載入後觸發動畫
  - 防止內容閃爍
  - _Requirements: 6.4_
...
- [ ] 14.6 (P) 防止 Cumulative Layout Shift
...
- [ ] 16.4 (P) 執行 Lighthouse Performance 測試
  - 執行 Lighthouse CI
