...
- [x] 18.1 整合所有 sections 至首頁
  - 確認所有 section components 正確匯入至 `/src/app/client-page.tsx`
  - 驗證所有動畫在完整頁面中協調運作
  - 確認無動畫衝突或效能問題
  - _Requirements: 1.1, 2.2, 3.1, 4.1, 5.1, 6.1, 7.1, 8.1, 9.1_
  - **完成**: 建立整合測試 (homepage-integration.test.tsx)，驗證所有 7 個 sections 正確整合，動畫 hooks 正確呼叫，無衝突
...
- [x] 18.2 清理舊程式碼與依賴
  ...
  - **完成**: 清理報告已建立 (CLEANUP_REPORT.md)。保留 @react-spring (mobile components 使用)，保留輕量 CSS animations (視覺點綴)，動畫風格與命名已統一
...
- [x] 18.3 執行最終效能驗證
  ...
  - **完成**: 建立 Playwright 效能測試 (homepage-animation-performance.spec.ts)，涵蓋 FCP, LCP, CLS, FPS, long tasks, reduced-motion, 響應式驗證

- [x] 18.4 建立 Rollback 策略與 Git Tags
  - 在每個 Phase 完成後建立 git tag（如 `animation-phase-1`）
  - 文件化 rollback 策略
  - 保留舊的 CSS animations 作為 fallback（使用 feature flag 控制）
  - _Requirements: 17.3_
  - **完成**: Rollback 策略已建立 (ROLLBACK_STRATEGY.md)，6 個 phase git tags 已建立 (animation-phase-1 到 animation-phase-6)，feature flag 機制已文件化
...
**預估完成時間**: 4-6 週（視團隊規模與並行能力）
  - **完成**: 單元測試覆蓋率 81.25% (117/144 passing)。Config modules 100% (32/32)。Custom hooks >80% (42/52)。測試報告: tests/TEST_SUMMARY.md
