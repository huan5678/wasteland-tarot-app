 ...
 * Displays list of weekly tasks with progress tracking
 */
...
export function WeeklyTaskList() {
  const { weeklyTasks, isLoading, error, claimWeeklyTaskReward, isClaimingWeekly } = useTasksStore();
      ...
      <div className="flex items-center justify-center py-12">
        <div className="flex flex-col items-center gap-3">
          <PixelIcon
            ...
            decorative
          />
          <p className="text-sm text-pip-boy-green/70">載入每週任務中...</p>
        </div>
      </div>
    );
  }
  ...
  if (!weeklyTasks) {
    return (
      <div className="flex items-center justify-center py-12">
        <p className="text-sm text-pip-boy-green/70">無每週任務資料</p>
      </div>
    );
  }

  const { tasks, completed_count, total_count } = weeklyTasks;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-2">
          <PixelIcon name="calendar-week" sizePreset="sm" variant="primary" decorative />
      ...
      <div className="space-y-3">
        {tasks.map((task) => (
          <TaskItem
            ...
            task={task}
            type="weekly"
            onClaimReward={claimWeeklyTaskReward}
            isClaiming={isClaimingWeekly[task.id] || false}
          />
        ))}
      </div>
