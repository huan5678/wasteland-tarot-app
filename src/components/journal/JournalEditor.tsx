    ...
    <div className="journal-editor space-y-6" role="form">
      ...
      <div className="flex gap-2 border-b border-pip-boy-green pb-2">
        <button
          ...
          className={`px-4 py-2 font-cubic text-sm transition-colors ${
            mode === 'edit'
...
- 列表項目"
          ...
          {MOOD_TAGS.map(({ value, label }) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 text-xs font-cubic border transition-colors ${
                  isSelected
                    ? 'bg-pip-boy-green text-wasteland-dark border-pip-boy-green selected'
                    : 'text-pip-boy-green border-pip-boy-green/50 hover:border-pip-boy-green hover:bg-pip-boy-green/10'
                }`}
                aria-label={label}
              >
                {label}
      ...
      <div className="flex gap-3 pt-4 border-t border-pip-boy-green">
        <button
          ...
          onClick={handleSave}
          disabled={!canSave}
          className="px-6 py-2 bg-pip-boy-green text-wasteland-dark font-cubic text-sm hover:bg-pip-boy-green/90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
          aria-label="儲存日記"
        >
        ...
        <button
          ...
          onClick={onCancel}
          disabled={isSaving}
          className="px-6 py-2 border border-pip-boy-green text-pip-boy-green font-cubic text-sm hover:bg-pip-boy-green/10 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          aria-label="取消"
        >
          取消
        </button>
