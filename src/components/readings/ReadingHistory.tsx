    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
        <div className="flex flex-col gap-1">
          <input placeholder="標籤過濾 (逗號分隔)" value={tagFilter} onChange={e=>setTagFilter(e.target.value)} className="px-3 py-2 bg-black border border-pip-boy-green text-pip-boy-green text-sm" />
          <div className="flex items-center gap-2">
            <select value={tagMode} onChange={e=>setTagMode(e.target.value as any)} className="px-2 py-1 bg-black border border-pip-boy-green text-pip-boy-green text-[10px]">
              <option value="any">任一</option>
              <option value="all">全部</option>
            </select>
            <div className="flex-1">
              <select onChange={e=>{ const v=e.target.value; if(!v)return; setTagFilter(tf=> tf ? (tf.split(',').map(t=>t.trim()).includes(v)? tf : tf+','+v) : v) }} className="w-full px-2 py-1 bg-black border border-pip-boy-green text-pip-boy-green text-[10px]">
        ...
        <input placeholder="標籤過濾 (完整匹配)" value={tagFilter} onChange={e=>setTagFilter(e.target.value)} className="px-3 py-2 bg-black border border-pip-boy-green text-pip-boy-green text-sm" />
        ...
        <select value={filter} onChange={e=>setFilter(e.target.value as any)} className="px-3 py-2 bg-black border border-pip-boy-green text-pip-boy-green text-sm">
          <option value="all">全部</option>
