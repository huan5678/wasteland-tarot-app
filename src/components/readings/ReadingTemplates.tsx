  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-pip-boy-green font-mono">占卜模板</h2>
          <p className="text-pip-boy-green/70 font-mono text-sm">預設問題模板，快速開始占卜</p>
        </div>
        <button
          onClick={startCreate}
      ...
      {(isCreating || editingTemplate) && (
        <div className="border-2 border-pip-boy-green/30 bg-pip-boy-green/5 p-6 space-y-4">
          <h3 className="font-mono font-bold text-pip-boy-green">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="block text-pip-boy-green font-mono text-sm">模板名稱 *</label>
              <input
                type="text"
                value={formData.name || ''}
            ...
            {/* Icon & Color */}
            <div className="space-y-1">
              <label className="block text-pip-boy-green font-mono text-sm">圖示與顏色</label>
          ...
          {/* Question */}
          <div className="space-y-1">
            <label className="block text-pip-boy-green font-mono text-sm">預設問題 *</label>
            <textarea
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Spread Type */}
            <div className="space-y-1">
              <label className="block text-pip-boy-green font-mono text-sm">牌陣類型</label>
              <select
            ...
            {/* Category */}
            <div className="space-y-1">
              <label className="block text-pip-boy-green font-mono text-sm">預設類別</label>
              <select
          ...
          {/* Tags */}
          <div className="space-y-1">
            <label className="block text-pip-boy-green font-mono text-sm">預設標籤</label>
            <input
              type="text"
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-mono font-bold text-pip-boy-green">{template.name}</div>
              ...
              <div className="space-y-2">
                <p className="font-mono text-sm text-pip-boy-green/80">{template.description}</p>

                {selectedTemplate?.id === template.id && (
                  <div className="border-t border-pip-boy-green/30 pt-3 space-y-3">
                    <div>
                      <div className="font-mono text-xs text-pip-boy-green/70 mb-1">問題:</div>
                      <div className="font-mono text-sm text-pip-boy-green italic">"{template.question}"</div>
                    </div>

                    {template.tags.length > 0 && (
                      <div>
                        <div className="font-mono text-xs text-pip-boy-green/70 mb-1">標籤:</div>
                        <div className="flex flex-wrap gap-1">
                          {template.tags.map(tag => (
                            <span key={tag} className="px-2 py-0.5 bg-pip-boy-green/20 border border-pip-boy-green/30 text-pip-boy-green font-mono text-xs">
                              {tag}
                            </span>
                          ))}
                        </div>
                    ...
                    <button
                      ...
                      className="w-full px-3 py-2 border border-pip-boy-green bg-pip-boy-green/10 text-pip-boy-green
                               font-mono text-sm hover:bg-pip-boy-green/20 flex items-center justify-center gap-2"
                    >
              ...
              {usageCount > 0 && (
                <div className="flex items-center gap-1 text-pip-boy-green/60 font-mono text-xs">
      ...
      {templates.length === 0 && (
        <div className="text-center py-12 border-2 border-pip-boy-green/30">
          ...
          <div className="font-mono text-lg font-bold text-pip-boy-green/70 mb-2">尚無模板</div>
          <div className="font-mono text-sm text-pip-boy-green/50 mb-4">建立你的第一個占卜模板</div>
          <button
            onClick={startCreate}
            className="px-4 py-2 border border-pip-boy-green/30 text-pip-boy-green font-mono text-sm
