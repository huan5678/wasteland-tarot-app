/**
 * WebGL Quad-based Donut Renderer
 *
 ...
 * More efficient than point-based rendering as it eliminates CPU readPixels overhead.
 */
...
import { getQuadShaderSources } from './quadShaders';
 ...
 */
export class WebGLQuadDonutRenderer {
  ...
  // Buffers
  private quadVertexBuffer: WebGLBuffer; // Quad corner vertices [0,0], [1,0], [0,1], [1,1]
  private quadIndexBuffer: WebGLBuffer; // Quad indices for 2 triangles
  private torusBuffer: WebGLBuffer; // Torus parametric coordinates
  private torusCount: number; // Number of torus points
  ...
  // Rendering mode (instanced or batched)
  ...
  constructor(config: DonutRendererConfig) {
    ...
    const contextResult = createWebGLContext(this.canvas, {
      preferWebGL2: true,
      alpha: true, // Enable alpha for transparent characters
      antialias: false,
      preserveDrawingBuffer: false, // Not needed for quad rendering
    });

    if (!contextResult) {
      throw new Error('[WebGLQuadDonutRenderer] WebGL is not supported');
    }
    ...
    console.log(
      `[WebGLQuadDonutRenderer] Initialized with WebGL ${contextResult.version}.0` +
    ...
    this.torusBuffer = this.createTorusCoordinates();
   ...
   * @returns Empty string (quad rendering doesn't output ASCII text)
   */
  render(angleA: number, angleB: number): string {
    ...
    if (this.useInstancing && this.gl2) {
      ...
      this.gl2.drawElementsInstanced(
        ...
        gl.UNSIGNED_SHORT,
        0,
        this.torusCount // Number of instances
      );
    } else {
      // WebGL 1.0: Draw each quad individually
      // Note: This is significantly slower than instancing
      for (let i = 0; i < this.torusCount; i++) {
        // Update per-instance data (would need to be done via uniforms in WebGL 1.0)
        // For now, we'll skip this and just note it needs implementation
    ...
    // For compatibility with Canvas 2D API, return empty string
    // (quad rendering displays directly on canvas, no ASCII text output)
    return '';
  }
  ...
  dispose(): void {
    ...
    gl.deleteBuffer(this.torusBuffer);
    gl.deleteTexture(this.charTexture);
    gl.deleteProgram(this.program);

    console.log('[WebGLQuadDonutRenderer] Resources disposed');
  }
  ...
  private createTexture(): WebGLTexture {
    ...
    if (!texture) {
      throw new Error('[WebGLQuadDonutRenderer] Failed to create texture');
    }
    return texture;
  }
  ...
  private createShaderProgram(): WebGLProgram {
    ...
    const shaderSources = getQuadShaderSources(version);
    ...
    if (!program) {
      throw new Error('[WebGLQuadDonutRenderer] Failed to create shader program');
    }
  ...
  private createQuadVertices(): WebGLBuffer {
    ...
    if (!buffer) {
      throw new Error('[WebGLQuadDonutRenderer] Failed to create quad vertex buffer');
    }
  ...
  private createQuadIndices(): WebGLBuffer {
    ...
    if (!buffer) {
      throw new Error('[WebGLQuadDonutRenderer] Failed to create quad index buffer');
    }
   ...
   *
   * FIXED: Changed from torus parametric coordinates (28,350 points)
   * to ASCII grid coordinates (width × height points)
   */
  private createTorusCoordinates(): WebGLBuffer {
    ...
    this.torusCount = coords.length / 2;
    ...
    if (!buffer) {
      throw new Error('[WebGLQuadDonutRenderer] Failed to create grid buffer');
    }
